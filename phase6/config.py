...
@dataclass
class Phase6Config:
    run_name: str = "phase6_robust_lcg"
    dt: float = 0.01
    steps: int = 2000
    delay_steps: int = 2
    mass: float = 1.0
    ...
    random_ref_count: int = 8
    domain_mass_range: Tuple[float, float] = (0.7, 1.4)
    domain_friction_range: Tuple[float, float] = (0.2, 0.6)
    domain_drag_mult_range: Tuple[float, float] = (4.0, 9.0)
    domain_bias_scale_range: Tuple[float, float] = (2.0, 4.5)
    domain_delay_steps_range: Tuple[int, int] = (1, 4)
    safety_window: int = 25
    safety_sat_rate_hi: float = 0.25
    safety_sat_rate_lo: float = 0.1
    safety_dr_scale: float = 0.8
    safety_a_scale: float = 0.85
    safety_hold_steps: int = 20
    adapt_lr: float = 0.02
    adapt_target_err: float = 0.06
    adapt_a_min: float = 0.55
    adapt_a_max: float = 1.15
    ...
    def __post_init__(self) -> None:
        ...
        if self.eval_sine_freqs is None:
            self.eval_sine_freqs = [0.1, 0.2, 0.3, 0.5]
