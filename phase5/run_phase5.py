...
from .env_runner import reference_signal, initial_state, rollout, step_dynamics
...
def collect_dagger_data(config: Phase5Config, action_grid: ActionGrid, model) -> Tuple[np.ndarray, np.ndarray]:
    xs_all = []
    ys_all = []
    for ds in config.dist_scales:
        for ref_type, ref_param in [
...
def write_summary(
    ...
    with open(out_path, "w") as f:
        ...
        f.write("## Dataset\n")
        f.write(f"- Samples: {train_metrics['n_train'] + train_metrics['n_val'] + train_metrics['n_test']}\n")
        f.write(f"- Train/Val/Test: {train_metrics['n_train']} / {train_metrics['n_val']} / {train_metrics['n_test']}\n")
        ...
        f.write("| test | dist_scale | policy | sat_total | RMSE | recovery | mean|e| | max|e| | %time(|e|<eps) |\n")
        f.write("|---|---:|---|---:|---:|---:|---:|---:|---:|\n")
        for row in eval_rows:
            f.write(
                "| {test} | {dist} | {policy} | {sat_total:.2%} | {rmse:.4f} | {recovery:.4f} | {mean_abs:.4f} | {max_abs:.4f} | {in_band:.2%} |\n".format(
                    **row
                )
            )
...
def write_eval_csv(out_dir: Path, eval_rows: List[Dict]) -> None:
    ...
    columns = ["test", "dist", "policy", "sat_total", "rmse", "recovery", "mean_abs", "max_abs", "in_band"]
    with open(out_path, "w") as f:
...
def main() -> None:
    ...
    dagger_stats = {"iterations": config.dagger_iterations, "samples": len(X_dag) if config.dagger_iterations else 0}
