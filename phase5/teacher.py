...
from .env_runner import EnvState, clamp, reference_signal, step_dynamics
...
class ActionGrid:
    ...
    @classmethod
    def from_config(cls, config: Phase5Config) -> "ActionGrid":
        actions = [(dr, a) for dr in config.dr_grid for a in config.a_grid]
        return cls(actions=actions)


def apply_action(r: float, state: EnvState, dr_max: float, a_scale: float, config: Phase5Config) -> Tuple[float, EnvState]:
    r_scaled = a_scale * r
...
def horizon_teacher_action(
    ...
    for idx, (dr_max, a_scale) in enumerate(action_grid.actions):
        ...
        sim_state, err, sat = step_dynamics(sim_state, r_use, dist_scale, config, t_step)
        ...
        for h in range(1, config.teacher_horizon):
            ...
            sim_state, err_h, sat_h = step_dynamics(sim_state, r_use_h, dist_scale, config, t_step + h)
